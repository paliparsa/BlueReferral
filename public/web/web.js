...
(function() {
  ...
  function openAccountModal(initialTab = 'subs') {
    ...
    try {
      ...
      if ($('accUserName')) $('accUserName').textContent = u.first_name || u.username || 'حساب کاربری';
      if ($('accUserEmail')) $('accUserEmail').textContent = u.email || `@${u.username || ''}`;
      ...
      if ($('walletBalanceText')) $('walletBalanceText').textContent = `${nf(u.balance)} تومان`;
      ...
      if ($('refTotalEarned')) $('refTotalEarned').textContent = `${nf(u.total_earned)} تومان`;
      if ($('refCount')) $('refCount').textContent = `${nf(u.referrals_count)} نفر`;
