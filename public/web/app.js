// BlueGate Web Store — Web Mode (adapted from MiniApp)
// Telegram WebApp is shimmed to null; all tg?.xxx calls safely no-op
const tg = window.Telegram?.WebApp || null;
const initData = ''; // Web mode: no Telegram initData
...
const adminFlag = getUrlFlag('admin') || getUrlFlag('mode') || getUrlFlag('startapp') || (location.pathname.toLowerCase().includes('/admin') ? '1' : '') || tg?.initDataUnsafe?.start_param || '';
...
async function api(action, payload = {}) {
  ...
  const candidates = ['/api.php', 'api.php', '../api.php'];
  let lastErr = null;
  for (const ep of candidates) {
    try {
      const res = await fetch(ep, {
        ...
        body: JSON.stringify({ action, initData: '', authToken, ...payload })
      });
      ...
      // Expose state globally for web-init.js
      if (action === 'me' || action === 'admin_summary') window.state = data;
      return data;
    } catch (err) {
      lastErr = err;
      if (err.error === 'AUTH_REQUIRED' || err.error === 'INVALID_CREDENTIALS' || err.status === 403) throw err;
      if (ep !== candidates[candidates.length - 1]) continue;
    }
  }
...
function sectionHtml(title,products){return `<section class="catalog-category-block"><h2 class="category-block-title">${title}</h2><div class="product-grid-wrap">${products.map(productCard).join('')}</div></section>`}
function gridHtml(products){return products.length ? `<section class="catalog-category-block"><div class="product-grid-wrap">${products.map(productCard).join('')}</div></section>` : ''}
...
// ── Web Mode: expose navigation globals for web-init.js ──
try {
  window.setTab = typeof setTab === 'function' ? setTab : null;
  window.renderUser = typeof renderUser === 'function' ? renderUser : null;
  window.renderAdmin = typeof renderAdmin === 'function' ? renderAdmin : null;
} catch(e) {}
