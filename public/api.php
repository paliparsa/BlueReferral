...
function product_payload(array $p, bool $activeVariants=true): array {
    $variants = array_map(function($v){ return ['id'=>(int)$v['id'], 'product_id'=>(int)$v['product_id'], 'title'=>$v['title'], 'price'=>(int)$v['price'], 'duration_days'=>(int)$v['duration_days'], 'sort_order'=>(int)($v['sort_order'] ?? 0), 'is_active'=>(int)($v['is_active'] ?? 1)]; }, product_variants((int)$p['id'], $activeVariants));
    return [
        ...
        'name'=>$p['name'], 'price'=>(int)$p['price'], 'price_label'=>product_price_label($p),
        'short_description'=>$p['short_description'], 'full_description'=>$p['full_description'], 'image_url'=>$p['image_url'] ?? null,
...
function admin_payload(): array {
    return ['ok'=>true,
        ...
        'settings'=>['payment_instructions'=>setting('payment_instructions',''), 'payment_methods_enabled'=>setting_json('payment_methods_enabled', ['wallet'=>true,'card'=>true,'stars'=>false,'crypto'=>false]), 'payment_methods'=>payment_methods_public(null), 'card_accounts_text'=>card_accounts_lines(), 'stars_rate_toman'=>setting_int('stars_rate_toman', 3200), 'swapwallet_has_api_key'=>swapwallet_api_key() !== '', 'swapwallet_application'=>setting('swapwallet_username', setting('swapwallet_application','')), 'swapwallet_username'=>setting('swapwallet_username', setting('swapwallet_application','')), 'swapwallet_base_url'=>setting('swapwallet_base_url','https://swapwallet.app/api'), 'swapwallet_auto_token'=>setting('swapwallet_auto_token','USDT'), 'swapwallet_usdt_rate_toman'=>(float)setting('swapwallet_usdt_rate_toman',0), 'swapwallet_rate_markup_percent'=>(float)setting('swapwallet_rate_markup_percent','1'), 'swapwallet_ttl_minutes'=>setting_int('swapwallet_ttl_minutes',30), 'swapwallet_notify_fail'=>setting_bool('swapwallet_notify_fail', true), 'theme_color'=>setting('theme_color','#1d9bf0'), 'button_colors_enabled'=>setting_bool('button_colors_enabled', true), 'button_colors'=>button_colors(), 'require_contact_auth'=>setting_bool('require_contact_auth', false), 'notify_new_user'=>setting_bool('notify_new_user', true), 'spin_referrals_per_chance'=>setting_int('spin_referrals_per_chance', 5), 'spin_rewards_text'=>spin_rewards_lines()]
    ];
}
...
if ($action === 'start_swapwallet_invoice' || $action === 'select_crypto_wallet') { try{ $order=start_swapwallet_invoice((int)($input['order_id']??0),(int)$user['id']); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id'])) + ['order'=>order_public_payload($order)]); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'امکان ساخت لینک پرداخت SwapWallet نیست. API Key، Application یا نرخ دستی را بررسی کن.'],400); } }
if ($action === 'check_swapwallet_invoice' || $action === 'check_crypto_payment') { try{ swapwallet_refresh_invoice((int)($input['order_id']??0)); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id']))); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'بررسی پرداخت SwapWallet انجام نشد.'],400); } }
// Legacy raw TXID crypto flow is intentionally disabled for new payments.
if ($action === 'submit_crypto_hash') { api_out(['ok'=>false,'error'=>'LEGACY_TXID_DISABLED','message'=>'پرداخت رمزارز به SwapWallet منتقل شده و نیاز به ارسال TXID نیست.'],400); }
...
if ($action === 'submit_receipt') { $orderId=(int)($input['order_id']??0); $note=trim((string)($input['note']??'')); if(mb_strlen($note)<3) api_out(['ok'=>false,'error'=>'EMPTY_RECEIPT','message'=>'توضیح رسید پرداخت را کامل‌تر بنویس.'],400); try{ $order=submit_order_receipt($orderId,(int)$user['id'],$note,null); notify_admins(order_admin_card($order)."\n\nرسید/توضیح پرداخت از Mini App:\n".h($note)); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id'])) + ['order'=>order_public_payload($order)]); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>'ORDER_NOT_FOUND','message'=>'سفارش پیدا نشد یا قابل پرداخت نیست.'],400); } }
...
if ($action === 'admin_save_settings') {
    ...
    if(isset($input['swapwallet_api_key']) && trim((string)$input['swapwallet_api_key']) !== '') set_setting('swapwallet_api_key', trim((string)$input['swapwallet_api_key']));
    if(isset($input['swapwallet_application'])) { $swu=trim((string)$input['swapwallet_application']); set_setting('swapwallet_application', $swu); set_setting('swapwallet_username', $swu); }
    if(isset($input['swapwallet_username'])) { $swu=trim((string)$input['swapwallet_username']); set_setting('swapwallet_username', $swu); set_setting('swapwallet_application', $swu); }
    if(isset($input['swapwallet_base_url'])) set_setting('swapwallet_base_url', rtrim(trim((string)$input['swapwallet_base_url']), '/') ?: 'https://swapwallet.app/api');
    if(isset($input['swapwallet_auto_token'])) set_setting('swapwallet_auto_token', strtoupper(trim((string)$input['swapwallet_auto_token']) ?: 'USDT'));
    if(isset($input['swapwallet_usdt_rate_toman'])) set_setting('swapwallet_usdt_rate_toman', (string)max(0,(float)$input['swapwallet_usdt_rate_toman']));
    if(isset($input['swapwallet_rate_markup_percent'])) set_setting('swapwallet_rate_markup_percent', (string)max(0,(float)$input['swapwallet_rate_markup_percent']));
    if(isset($input['swapwallet_ttl_minutes'])) set_setting('swapwallet_ttl_minutes', max(5,(int)$input['swapwallet_ttl_minutes']));
    if(isset($input['swapwallet_notify_fail'])) set_setting('swapwallet_notify_fail', bool_input($input['swapwallet_notify_fail'])?'1':'0');
...
if ($action === 'admin_add_variant') { require_admin($user); $pid=(int)($input['product_id']??0); $title=trim((string)($input['title']??'')); $price=max(0,(int)($input['price']??0)); if($pid<=0||$title===''||$price<=0) api_out(['ok'=>false,'message'=>'محصول، نام پلن و قیمت الزامی است.'],400); db()->prepare('INSERT INTO product_variants (product_id,title,price,duration_days,sort_order,is_active) VALUES (?,?,?,?,?,1)')->execute([$pid,$title,$price,max(0,(int)($input['duration_days']??0)),max(0,(int)($input['sort_order']??99))]); api_out(admin_payload()); }
if ($action === 'admin_update_variant') { require_admin($user); $id=(int)($input['variant_id']??0); foreach(['title','price','duration_days','sort_order','is_active'] as $f){ if(array_key_exists($f,$input)) update_variant_field($id,$f,$input[$f]); } api_out(admin_payload()); }
