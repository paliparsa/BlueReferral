...
if ($action === 'admin_add_product') { require_admin($user); $name=trim((string)($input['name']??'')); try{$pp=price_admin_payload_from_input($input);}catch(Throwable $e){api_out(['ok'=>false,'message'=>'قیمت معتبر نیست یا نرخ USDT برای قیمت دلاری در دسترس نیست.'],400);} if($name==='') api_out(['ok'=>false,'message'=>'نام محصول الزامی است.'],400); $catId=!empty($input['category_id']) ? (int)$input['category_id'] : null; $delivery=normalize_delivery_type((string)($input['delivery_type']??'manual')); $commissionType=in_array(($input['commission_type']??'none'),['none','fixed','percent'],true)?$input['commission_type']:'none'; $commissionValue=max(0,(int)($input['commission_value']??0)); $flashDiscount=max(0,(int)($input['flash_sale_discount']??0)); $flashStart=(!empty($input['flash_sale_start'])&&strtotime((string)$input['flash_sale_start']))?date('Y-m-d H:i:s',strtotime((string)$input['flash_sale_start'])):null; $flashEnd=(!empty($input['flash_sale_end'])&&strtotime((string)$input['flash_sale_end']))?date('Y-m-d H:i:s',strtotime((string)$input['flash_sale_end'])):null; db()->prepare('INSERT INTO products (category_id,name,price,price_currency,price_usd,price_rate_toman,price_rate_source,price_rate_updated_at,short_description,full_description,image_url,delivery_type,commission_type,commission_value,duration_days,is_featured,is_active,flash_sale_start,flash_sale_end,flash_sale_discount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$catId ?: null,$name,$pp['price'],$pp['price_currency'],$pp['price_usd'],$pp['price_rate_toman'],$pp['price_rate_source'],$pp['price_rate_updated_at'],(string)($input['short_description']??''),(string)($input['full_description']??''),trim((string)($input['image_url']??'')) ?: null,$delivery,$commissionType,$commissionValue,max(0,(int)($input['duration_days']??0)),!empty($input['is_featured'])?1:0,1,$flashStart,$flashEnd,$flashDiscount]); api_out(admin_payload()); }
if ($action === 'admin_update_product') { require_admin($user); $id=(int)($input['product_id']??0); if(array_key_exists('price_currency',$input)||array_key_exists('price_usd',$input)||array_key_exists('price',$input)){ try{$pp=price_admin_payload_from_input($input); foreach($pp as $k=>$v) update_product_field($id,$k,$v);}catch(Throwable $e){api_out(['ok'=>false,'message'=>'قیمت معتبر نیست یا نرخ USDT برای قیمت دلاری در دسترس نیست.'],400);} } foreach(['category_id','name','short_description','full_description','image_url','delivery_type','commission_type','commission_value','duration_days','is_active','is_featured','flash_sale_start','flash_sale_end','flash_sale_discount'] as $f){ if(array_key_exists($f,$input)) update_product_field($id,$f,$input[$f]); } api_out(admin_payload()); }
