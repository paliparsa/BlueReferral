...
if ($action === 'admin_delete_coupon') { require_admin($user); $cid=(int)($input['coupon_id']??0); try{ $list=admin_delete_coupon($cid); api_out(['ok'=>true,'coupons'=>$list]); }catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'خطا در حذف کد تخفیف.'],400); } }
