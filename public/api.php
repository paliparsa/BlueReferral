...
if ($action === 'select_crypto_wallet') { try{ $order=start_crypto_payment((int)($input['order_id']??0),(int)$user['id'],(int)($input['wallet_id']??0)); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id'])) + ['order'=>order_public_payload($order)]); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'امکان ساخت پرداخت رمزارز نیست. نرخ یا کیف پول را بررسی کن.'],400); } }
if ($action === 'submit_crypto_hash') { try{ $order=submit_crypto_hash((int)($input['order_id']??0),(int)$user['id'],(string)($input['tx_hash']??'')); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id'])) + ['order'=>order_public_payload($order)]); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'هش قابل ثبت نیست یا قبلاً استفاده شده است.'],400); } }
if ($action === 'check_crypto_payment') { try{ crypto_verify_order((int)($input['order_id']??0)); api_out(dashboard_payload(get_user_by_tid((int)$user['telegram_id']))); } catch(Throwable $e){ api_out(['ok'=>false,'error'=>$e->getMessage(),'message'=>'بررسی پرداخت انجام نشد.'],400); } }
