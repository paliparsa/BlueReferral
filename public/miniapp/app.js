...
function renderShop(){const cats=state.shop_categories||[];const brand=state.brand||'BlueReferral';$('shopPage').innerHTML=`<section class="shop-hero"><div><small>${esc(brand)}</small><h2>محصولاتو راحت پیدا کن</h2></div><span>🛍</span></section><div class="searchbar">🔍<input id="searchInput" autocomplete="off" inputmode="search" placeholder="جستجوی محصول، اشتراک، VPN..." value="${esc(searchTerm)}"><div class="search-actions"><button class="ghost" id="toggleCardMode" data-toggle-card-mode>${productCardMode==='compact'?'🔳':'📋'}</button></div></div><div class="shop-filter-bar"><div class="shop-sort-group"><button class="shop-filter-chip ${shopSort==='newest'?'active':''}" data-shop-sort="newest">جدیدترین</button><button class="shop-filter-chip ${shopSort==='price_low'?'active':''}" data-shop-sort="price_low">ارزان‌ترین</button><button class="shop-filter-chip ${shopSort==='price_high'?'active':''}" data-shop-sort="price_high">گران‌ترین</button></div><div class="shop-toggle-group"><button class="shop-filter-chip ${shopFilterWishlist?'active':''}" data-shop-toggle="wishlist">${shopFilterWishlist?'❤️ ':'🤍 '}نشان‌شده</button><button class="shop-filter-chip ${shopFilterInStock?'active':''}" data-shop-toggle="instock">${shopFilterInStock?'✓ ':''}فقط آنی</button><button class="shop-filter-chip ${shopFilterFeatured?'active':''}" data-shop-toggle="featured">${shopFilterFeatured?'✓ ':''}ویژه</button></div></div><div class="category-strip"><button class="cat-pill ${activeCategory==='all'?'active':''}" data-cat="all"><span>✨</span><b>همه</b></button><button class="cat-pill ${activeCategory==='featured'?'active':''}" data-cat="featured"><span>⭐</span><b>ویژه</b></button>${cats.map(c=>`<button class="cat-pill ${Number(activeCategory)===Number(c.id)?'active':''}" data-cat="${c.id}">${c.image_url?`<img src="${esc(c.image_url)}">`:`<span>${esc(c.emoji||'🛒')}</span>`}<b>${esc(c.title)}</b></button>`).join('')}</div><div id="shopSections">${shopSectionsHtml()}</div>`}
