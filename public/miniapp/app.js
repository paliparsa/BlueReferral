...
function orderExpiryMinutes(){
  return Number(state?.payment_expiry_minutes || adminState?.settings?.payment_expiry_minutes || 20);
}

function parseOrderDateMs(dateStr){
  ...
  if(parts.length >= 5){
    d = new Date(parts[0], parts[1]-1, parts[2], parts[3], parts[4], parts[5]||0);
    if(!isNaN(d.getTime())) return d.getTime();
  }
  return null;
}
...
function orderExpiryMs(o){
  const createdMs = parseOrderDateMs(o.created_at);
  if(!createdMs) return null;
  const expiryMins = orderExpiryMinutes();
  return createdMs + (expiryMins * 60 * 1000);
}
...
function renderAdminSettings(){
  ...
  return `<section class="settings-dashboard better-settings">
    ...
    <div class="settings-subtab-pane ${isPay?'':'hidden'}" data-pane="payments">
      <article class="settings-card admin-card">
        <div class="admin-card-head"><div class="admin-card-icon"><span>💳</span></div><div><h3>روش‌های فعال پرداخت</h3><p class="muted">روش‌هایی که در مرحله نهایی فاکتور به کاربر پیشنهاد می‌شوند.</p></div></div><div class="form-grid settings-form" style="margin-bottom:14px"><label class="full"><span>⏱️ مهلت انقضای فاکتور پرداخت (دقیقه)</span><input id="as_payment_expiry_minutes" type="number" inputmode="numeric" value="${esc(s.payment_expiry_minutes||20)}" placeholder="مثلاً 20"></label></div>
