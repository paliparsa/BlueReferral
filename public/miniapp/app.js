...
function orderRowHtml(o){const paid=Number(o.wallet_amount||0)>0?` · کیف پول ${fmt(o.wallet_amount)}`:'';return `<article class="order-row" data-order-open="${o.id}" style="flex-direction:column;align-items:stretch"><div class="order-row-main" style="margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;width:100%"><div class="order-icon">${o.image_url?`<img src="${esc(o.image_url)}">`:'🧾'}</div><div style="flex:1"><h3>#${nf(o.id)} · ${esc(o.display_name)}</h3><p class="muted">${esc(o.created_at||'')} · مانده ${fmt(o.final_amount)}${paid}</p></div><span class="chev" style="align-self:center;font-size:24px;color:var(--muted)">‹</span></div><div class="order-row-stepper" style="width:100%">${orderStepperHtml(o)}</div></article>`}
