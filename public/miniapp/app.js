...
let state = null, adminState = null, currentTab = 'home', currentAdminTab = 'dashboard', searchTerm = '', activeCategory = 'all', pendingDialog = null, pendingEdit = null, currentOrderId = null, orderFilter = 'all', lastSpinPrize = null, searchTimeout = null;
...
function sectionHtml(title,products){return `<section class="section-row"><div class="section-title"><h2>${title}</h2></div><div class="h-scroll product-grid-wrap">${products.map(productCard).join('')}</div></section>`}
...
function productCard(p){return `<article class="product-tile" data-product="${p.id}"><div class="tile-img">${cardImage(p,'🛍')}</div><div class="tile-body"><h3>${esc(p.name)}</h3>${p.short_description?`<p class="tile-desc">${esc(p.short_description)}</p>`:''}<div class="price-row-mini"><span class="price-pill">${priceLabel(p)}</span>${Number(p.inventory_available||0)>0?'<span class="soon">آنی</span>':''}</div></div></article>`}
function buyButtonsForProduct(p){const bal=Number(state.user?.balance||0);const walletHint=bal>0?`<div class="wallet-hint">💰 موجودی شما: <b>${fmt(bal)}</b>؛ می‌تونی ازش برای کم‌کردن فاکتور استفاده کنی.</div>`:'';if((p.variants||[]).length){return `${walletHint}<div class="variant-list">${(p.variants||[]).map(v=>`<div class="variant-card"><div><b>${esc(v.title)}</b><span>${priceLabel(v)}</span></div><button class="primary" data-buy="${p.id}" data-variant="${v.id}">ثبت سفارش</button>${bal>0?`<button class="secondary" data-buy-wallet="${p.id}" data-variant="${v.id}">استفاده از کیف پول</button>`:''}</div>`).join('')}</div>`}return `${walletHint}<div class="actions variant-list"><button class="primary pulse" data-buy="${p.id}">ثبت سفارش</button>${bal>0?`<button class="secondary" data-buy-wallet="${p.id}">خرید با کیف پول</button>`:''}</div>`}
function showProduct(pid){const p=(state.shop_products||[]).find(x=>Number(x.id)===Number(pid));if(!p)return;currentTab='product';hidePages();$('productPage').classList.remove('hidden');$('productPage').innerHTML=`<div class="detail-hero product-hero">${cardImage(p,'🛍')}</div><article class="detail-card product-detail"><button class="secondary" data-back-shop>بازگشت به فروشگاه</button><h2>${esc(p.name)}</h2><div class="product-price-row"><span class="big-price">${priceLabel(p)}</span><span class="badge live-price-badge">${p.price_currency==='USD'?'نرخ لحظه‌ای':'قیمت ثابت'}</span><span class="badge">${esc(p.delivery_type_fa)}</span><span class="badge">موجودی آماده: ${nf(p.inventory_available||0)}</span></div><div class="description-box">${textBlock(p.full_description||p.short_description||'بدون توضیح')}</div>${buyButtonsForProduct(p)}</article>`}
...
function paymentMethodsHtml(o){
  ...
  if(methods.crypto?.enabled && (o.payment_method==='crypto' || cryptoWallets.length)){
    ...
    if(o.payment_method!=='crypto'){
      html+=`<p class="muted">کیف پول موردنظر را انتخاب کن. نرخ‌ها از Wallex/Ramzinex/Nobitex یا نرخ دستی ادمین خوانده می‌شوند و با cron به‌روز می‌شوند.</p><div class="crypto-wallet-grid">`+cryptoWallets.map(w=>{const rate=Number(w.rate_toman||0);const markup=Number(methods.crypto?.markup_percent||0)/100;const amount=rate>0?((Number(o.final_amount||0)/rate)*(1+markup)).toFixed(6):null;return `<button class="crypto-wallet" data-select-crypto="${o.id}:${w.id}"><b>${esc(w.title||w.asset)}</b><span>${esc(w.network)} · ${esc(w.asset)}</span><em>${amount?`${amount} ${esc(w.asset)}`:'نرخ دستی لازم است'}</em><small>${rate?`نرخ: ${nf(rate)} تومان${w.rate_updated_at?' · '+esc(w.rate_updated_at):''}`:''}</small></button>`}).join('')+`</div>`;
    } else if(cryptoCheck){
