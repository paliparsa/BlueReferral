...
function productById(id){return (state.shop_products||[]).find(p=>Number(p.id)===Number(id))}
function variantById(product,id){return (product?.variants||[]).find(v=>Number(v.id)===Number(id))}
function confirmOrderSubmit(button){
  const product=productById(button?.dataset?.buy||button?.dataset?.buyWallet);
  const variant=variantById(product,button?.dataset?.variant);
  const title=[product?.name,variant?.title].filter(Boolean).join(' - ') || 'این سفارش';
  const price=variant?.price_label||product?.price_label||'';
  const wallet=button?.dataset?.buyWallet?'با استفاده از کیف پول':'';
  const msg=`ثبت سفارش ${title}${price?`\nمبلغ: ${price}`:''}${wallet?`\n${wallet}`:''}\n\nآیا تایید می‌کنی؟`;
  return new Promise(resolve=>{
    try{
      if(tg?.showConfirm){tg.showConfirm(msg, ok=>resolve(!!ok));return}
    }catch(e){}
