...
function paymentMethodsHtml(o){
  ...
  if(methods.crypto?.enabled && (o.payment_method==='crypto' || cryptoWallets.length)){
    ...
    if(o.payment_method!=='crypto'){
      html+=`<p class="muted">کیف پول موردنظر را انتخاب کن. نرخ‌ها از cache نوبیتکس یا نرخ دستی ادمین خوانده می‌شوند و هر دقیقه با cron به‌روز می‌شوند.</p><div class="crypto-wallet-grid">`+cryptoWallets.map(w=>{const rate=Number(w.rate_toman||0);const markup=Number(methods.crypto?.markup_percent||0)/100;const amount=rate>0?((Number(o.final_amount||0)/rate)*(1+markup)).toFixed(6):null;return `<button class="crypto-wallet" data-select-crypto="${o.id}:${w.id}"><b>${esc(w.title||w.asset)}</b><span>${esc(w.network)} · ${esc(w.asset)}</span><em>${amount?`${amount} ${esc(w.asset)}`:'نرخ دستی لازم است'}</em><small>${rate?`نرخ: ${nf(rate)} تومان${w.rate_updated_at?' · '+esc(w.rate_updated_at):''}`:''}</small></button>`}).join('')+`</div>`;
    } else if(cryptoCheck){
      const amountText=Number(cryptoCheck.expected_amount||0).toFixed(6)+' '+esc(cryptoCheck.asset);
      html+=`<div class="crypto-invoice live"><div class="full warning-box"><b>مبلغ دقیق پرداخت</b><p>دقیقاً <b>${amountText}</b> باید به ولت زیر برسد. کارمزد صرافی/شبکه بر عهده شماست و نباید از این مبلغ کم شود.</p><button class="secondary" data-copy="${Number(cryptoCheck.expected_amount||0).toFixed(6)}">کپی مبلغ</button></div><div><small>شبکه / ارز</small><b>${esc(cryptoCheck.network)} / ${esc(cryptoCheck.asset)}</b></div><div><small>نرخ مبنا</small><b>${cryptoCheck.rate_toman?nf(cryptoCheck.rate_toman)+' تومان':'-'}</b><small>${cryptoCheck.rate_source?esc(cryptoCheck.rate_source):''} ${cryptoCheck.rate_updated_at?'· '+esc(cryptoCheck.rate_updated_at):''}</small></div><div class="full"><small>آدرس ولت</small><code>${esc(cryptoCheck.address)}</code><button class="secondary" data-copy="${esc(cryptoCheck.address)}">کپی ولت</button></div>${cryptoCheck.tx_hash?`<div class="full"><small>TXID</small><code>${esc(cryptoCheck.tx_hash)}</code></div>`:''}<div class="full"><small>وضعیت بررسی</small><b>${cryptoCheck.status==='confirmed'?'✅ تایید شده':cryptoCheck.status==='pending'?'در حال بررسی':'در انتظار هش'}</b>${cryptoCheck.fail_reason?`<p class="muted">${esc(cryptoCheck.fail_reason)}</p>`:''}</div></div><div class="actions"><button class="primary" data-crypto-hash="${o.id}">ثبت TXID / Hash</button><button class="secondary" data-check-crypto="${o.id}">بررسی دوباره</button></div>`;
    }
    html+=`</div>`;
  }
  html+=`</article>`;
...
function renderAdminBackup(){
  const b=adminState.backup||{};
  const tables=b.tables||{};
  const rows=Object.entries(tables).map(([k,v])=>`<div class="builder-row compact"><div><b>${esc(k)}</b><small>${nf(v||0)} ردیف</small></div></div>`).join('')||'<p class="muted">اطلاعات جدول‌ها در دسترس نیست.</p>';
  return `<section class="settings-dashboard better-settings backup-dashboard">
    <article class="settings-hero admin-card">
      <div><small>Backup Center</small><h3>🧩 بکاپ و ریستور BlueReferral</h3><p class="muted">از دیتابیس فروشگاه، کاربران، سفارش‌ها، محصولات، تنظیمات، ولت‌ها و انبار بکاپ بگیر و بعداً از همین پنل ریستور کن.</p></div>
      <button class="primary" data-admin-download-backup>دانلود بکاپ</button>
    </article>
    <article class="settings-card admin-card">
      <div class="settings-card-head"><span>⬇️</span><div><h3>دانلود بکاپ</h3><p class="muted">فایل خروجی با فرمت <b>.json.gz</b> ساخته می‌شود. config.php، توکن بات و سورس‌کد داخل بکاپ نیستند؛ سورس را از GitHub نگه دار.</p></div></div>
      <div class="admin-actions"><button class="primary" data-admin-download-backup>ساخت و دانلود بکاپ جدید</button></div>
      <p class="muted">آخرین بکاپ: ${esc(b.last_created_at||'ندارد')} ${b.last_filename?`· ${esc(b.last_filename)}`:''}</p>
    </article>
    <article class="settings-card admin-card danger-zone">
      <div class="settings-card-head"><span>⬆️</span><div><h3>آپلود و ریستور بکاپ</h3><p class="muted">ریستور، دیتابیس فعلی را با فایل بکاپ جایگزین می‌کند. قبل از ریستور حتماً یک بکاپ جدید دانلود کن.</p></div></div>
      <input id="backupFileInput" type="file" accept=".json,.gz,.json.gz,.zip,application/json,application/gzip,application/zip">
      <div class="admin-actions"><button class="danger" data-admin-upload-backup>آپلود و ریستور</button></div>
      <p class="muted">آخرین ریستور: ${esc(b.last_restored_at||'ندارد')} · حداکثر فایل: ${nf(b.max_upload_mb||25)}MB</p>
    </article>
    <article class="settings-card admin-card">
      <div class="settings-card-head"><span>🗂️</span><div><h3>جدول‌های داخل بکاپ</h3><p class="muted">این‌ها جدول‌هایی هستند که از دیتابیس فعلی بکاپ گرفته می‌شوند.</p></div></div>
      <div class="builder-list">${rows}</div>
    </article>
  </section>`;
}
function downloadBase64File(filename,mime,base64){
  const bin=atob(base64); const len=bin.length; const bytes=new Uint8Array(len);
  for(let i=0;i<len;i++) bytes[i]=bin.charCodeAt(i);
  const blob=new Blob([bytes],{type:mime||'application/octet-stream'});
  const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=filename||'blue-referral-backup.json.gz'; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(url),1500);
}
function fileToBase64(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>{const s=String(r.result||'');resolve(s.includes(',')?s.split(',').pop():s)};r.onerror=()=>reject(r.error||new Error('FILE_READ_FAILED'));r.readAsDataURL(file);})}
async function adminDownloadBackup(){
  showStatus('در حال ساخت بکاپ...');
  const res=await api('admin_create_backup');
  adminState=res;
  const f=res.backup_file;
  if(!f||!f.content_base64) throw new Error('BACKUP_FILE_MISSING');
  downloadBase64File(f.filename,f.mime,f.content_base64);
  renderAdmin();
  showStatus('بکاپ دانلود شد');
}
async function adminUploadBackup(){
  const input=$('backupFileInput'); const file=input?.files?.[0];
  if(!file){showStatus('اول فایل بکاپ را انتخاب کن','error');return}
  const ok=confirm('هشدار جدی: ریستور بکاپ دیتابیس فعلی را جایگزین می‌کند. قبل از ادامه یک بکاپ جدید دانلود کرده‌ای؟');
  if(!ok)return;
  const typed=prompt('برای تایید ریستور، دقیقاً بنویس: RESTORE');
  if(typed!=='RESTORE'){showStatus('ریستور لغو شد','error');return}
  showStatus('در حال آپلود و ریستور بکاپ...');
  const content_base64=await fileToBase64(file);
  const res=await api('admin_restore_backup',{filename:file.name,content_base64,confirm:'RESTORE'});
  adminState=res;
  renderAdmin();
  showStatus('بکاپ ریستور شد');
}
