...
function showProduct(pid){const p=(state.shop_products||[]).find(x=>Number(x.id)===Number(pid));if(!p)return;currentTab='product';hidePages();$('productPage').classList.remove('hidden');$('productPage').innerHTML=`<div class="detail-hero product-hero" style="position:relative"><button class="icon-btn" data-back-shop style="position:absolute;top:16px;left:16px;z-index:5;background:rgba(0,0,0,0.5);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.15);width:42px;height:42px;display:flex;align-items:center;justify-content:center;font-size:26px;border-radius:50%;padding-bottom:4px">‹</button><button class="icon-btn" data-share-product="${p.id}" style="position:absolute;top:16px;right:16px;z-index:5;background:rgba(0,0,0,0.5);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.15);width:42px;height:42px;display:flex;align-items:center;justify-content:center;font-size:18px;border-radius:50%">🔗</button>${cardImage(p,'🛍')}</div><article class="detail-card product-detail"><h2>${esc(p.name)}</h2><div class="product-price-row"><span class="big-price">${priceLabel(p)}</span><span class="badge live-price-badge">${p.price_currency==='USD'?'نرخ لحظه‌ای':'قیمت ثابت'}</span><span class="badge">${esc(p.delivery_type_fa)}</span><span class="badge">موجودی آماده: ${nf(p.inventory_available||0)}</span></div><div class="description-box">${textBlock(p.full_description||p.short_description||'بدون توضیح')}</div>${buyButtonsForProduct(p)}</article>`;window.scrollTo({top:0,behavior:'instant'})}
