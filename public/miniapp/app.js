...
function paymentMethodsHtml(o){
  ...
  if(!cardAccounts.length && (state.payment_instructions || methods.card?.instructions)){
    ...
    if(m){
      cardAccounts = [{title:'کارت بانکی سفارشات', card: m[0], owner:'پشتیبانی', sheba:''}];
    }
  }
  if(!cardAccounts.length && methods.card?.enabled){
    cardAccounts = [{title:'کارت بانکی', card:'6037997412345678', owner:'پشتیبانی فروشگاه', sheba:''}];
  }
