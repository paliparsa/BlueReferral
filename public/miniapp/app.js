...
function variantItemHtml(v){
  return `<div class="admin-item variant-item">
    <div class="variant-item-head">
      ...
      <div class="variant-item-meta">${priceAdminSummary(v)} ${Number(v.discount_percent)>0?`<span class="chip-mini chip-featured">تخفیف: ${nf(v.discount_percent)}٪</span> `:''}· ${Number(v.is_active)?'<span class="chip-mini chip-active">فعال</span>':'<span class="chip-mini chip-off">غیرفعال</span>'} · مدت: ${nf(v.duration_days)} روز</div>
    </div>
