...
if(t.dataset.coupon){
  const oid = t.dataset.coupon;
  const o = orderById(oid);
  openEdit(`کد تخفیف سفارش #${oid}`, [{
    title: 'کد تخفیف',
    fields: [{
      id: 'coupon_code_val',
      label: 'کد تخفیف را وارد کنید',
      type: 'text',
      placeholder: 'مثلاً BLUE10',
      value: o?.coupon_code || ''
    }]
  }], async () => {
    const code = val('coupon_code_val')?.trim();
    if (!code) throw new Error('لطفاً کد تخفیف را وارد کنید');
    await loadAfterAction('apply_coupon', { order_id: oid, code: code });
    currentTab = 'orders';
    currentOrderId = oid;
    renderUser();
    showStatus('کد تخفیف اعمال شد');
  });
  return;
}
if(t.dataset.cancel){
