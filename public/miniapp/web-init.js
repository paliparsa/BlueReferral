...
(function () {
  ...
  window.openWebCommandPalette = function () {
    const cp = $('cmdPalette');
    if (!cp) return;
    ...
    // 1. Navigation items
    let commands = [
      { label: 'فروشگاه و محصولات', icon: '🛒', badge: 'تب اصلی', action: () => window.setTab?.('shop') },
      { label: 'سفارش‌های من', icon: '🧾', badge: 'پیگیری سفارش', action: () => window.setTab?.('orders') },
    ...
    if (window.state && Array.isArray(window.state.products)) {
      const prodCmds = window.state.products.map(p => ({
        label: p.title || p.name || 'محصول',
        icon: p.icon || '⚡',
        ...
        action: () => {
          if (typeof window.openProductModal === 'function') {
            ...
            if (searchInp) {
              searchInp.value = p.title || p.name;
  ...
  /* ── Global ⌘K Shortcut & Search trigger ── */
  ...
  $('webHeaderSearchBox')?.addEventListener('click', (e) => {
  ...
  /* ── Reflect tab changes in web sidebar & topbar ── */
  const _syncSidebar = () => {
  ...
  console.log('[WebInit] BlueGate Web Mode Phase 1 initialized ✅');
})();
